...
void Key_SetDriveMode(uint8 mode)
{
	...
	CyPins_SetPinDriveMode(Key_1, mode);
	CyPins_SetPinDriveMode(Key_2, mode);
	CyPins_SetPinDriveMode(Key_3, mode);
}
...
/* If interrupt is connected for this Pins component */ 
    ...
    *******************************************************************************/
    void Key_SetInterruptMode(uint16 position, uint16 mode)
    {
		...
		if((position & Key_1_INTR) != 0u) 
		{ 
			 Key_1_INTTYPE_REG = (uint8)mode; 
		} 
		if((position & Key_2_INTR) != 0u) 
		{ 
			 Key_2_INTTYPE_REG = (uint8)mode; 
		} 
		if((position & Key_3_INTR) != 0u) 
		{ 
			 Key_3_INTTYPE_REG = (uint8)mode; 
		}
    }
